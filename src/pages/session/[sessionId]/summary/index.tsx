...
const SummaryPage = () => {
	...
	useEffect(() => {
		fetch(`${process.env.NEXT_PUBLIC_BACKEND}api/sessions/` + sessionId)
			.then(res => res.json())
			.then(resData => setData(summaryData(
				resData.title,
			...
			<Container>
				<Layout>
					<InstructionWrapper>
						...
						<h5>{data.info.map(i =>
							<><InfoSection {...i} key={i.label} /><br /></>)}
						</h5>


						<p>{data.description}</p>
						<TableDisplay>{data.indivScores.map(s => <TableLayout><IndivScore {...s} key={s.label} /></TableLayout>)}</TableDisplay>
					</InstructionWrapper>

					<OverallScore {...data.overallScore} /><br />
					<p>My personal accomplishments:</p>
					<InputBox value={data.accomplishments} readOnly={true} />

					{data.buttons.map((b: IButtonData) => <Button onClick={() => router.push('/')} key={b.label}>{b.label}</Button>)}
				</Layout>
			</Container>
		</>
	)
